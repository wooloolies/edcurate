...
def test_to_sync_database_url_converts_asyncpg_urls() -> None:
    ...
    assert to_sync_database_url(async_url) == "postgresql://user:pass@localhost:5432/edcurate"
...
def test_settings_derives_sync_database_url() -> None:
    ...
    assert settings.DATABASE_URL_SYNC == "postgresql://user:pass@localhost:5432/edcurate"
...
def test_send_verification_email_or_raise_converts_provider_failures(monkeypatch) -> None:
